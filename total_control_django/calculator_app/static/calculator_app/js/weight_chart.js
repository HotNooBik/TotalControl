...
document.addEventListener("DOMContentLoaded", function () {
                        ...
                        plugins: {
                            ...
                            zoom: {
                                limits: {
                                    ...
                                    y: {
                                        min: Math.round(
                                            Math.min(...data_points) - 2
                                        ),
                                        max: Math.round(
                                            Math.max(...data_points) + 2
                                        ),
                                    },
                                },
                                pan: {
    ...
    filterButtons.all.addEventListener("click", () =>
        handleFilterClick("all", filterButtons.all)
    );
    filterButtons.week.addEventListener("click", () =>
        handleFilterClick("week", filterButtons.week)
    );
    filterButtons.month.addEventListener("click", () =>
        handleFilterClick("month", filterButtons.month)
    );
    filterButtons.year.addEventListener("click", () =>
        handleFilterClick("year", filterButtons.year)
    );
        ...
        try {
            fetch(urls.getWeightHistory, {
                ...
                .then((data) => {
                    ...
                    chartFullInstance.options.scales.x.min =
                        lastTenIndexStart >= 0 ? lastTenIndexStart : 0;
                    chartFullInstance.options.scales.x.max =
                        data.labels.length - 1;

                    chartFullInstance.options.scales.y.min = Math.round(
                        Math.min(...data.data_points) - 1
                    );
                    chartFullInstance.options.scales.y.max = Math.round(
                        Math.max(...data.data_points) + 1
                    );

                    chartFullInstance.options.plugins.zoom.limits.x.max =
                        data.labels.length - 1;
                    chartFullInstance.options.plugins.zoom.limits.y.min =
                        Math.round(Math.min(...data.data_points) - 3);
                    chartFullInstance.options.plugins.zoom.limits.y.max =
                        Math.round(Math.max(...data.data_points) + 3);
