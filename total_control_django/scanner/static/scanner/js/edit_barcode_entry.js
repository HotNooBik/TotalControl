...
document.getElementById('amount-input').addEventListener('input', function (event) {
    ...
    if (!isNaN(amount)) {
        ...
        document.getElementById('current-amount').textContent = amount.toFixed(1);

        // Пересчет значений
        const multiply = (value) => (value * amount).toFixed(1);
        amount /= 100
        document.getElementById('calculated-calories').textContent = Math.round(multiply(calories));
        document.getElementById('calculated-proteins').textContent = multiply(proteins);
        document.getElementById('calculated-fats').textContent = multiply(fats);
        document.getElementById('calculated-carbs').textContent = multiply(carbs);
